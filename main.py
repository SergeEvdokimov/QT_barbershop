...
from variables import ChooseDateWidget as chooseDateUI, FiltersWidget as filtersUi, weekdays, months
...
class ChooseDateWidget(QMainWindow):
    def __init__(self, parent=None):
        ...
        # подключении интерфейса из chooseDateUI
        ui = io.StringIO(chooseDateUI)
        uic.loadUi(ui, self)
...
class AddFiltersWidget(QMainWindow):
    def __init__(self, parent=None):
        ...
        # подключение интерфейса из переменной filtersUi
        ui = io.StringIO(filtersUi)
        uic.loadUi(ui, self)
